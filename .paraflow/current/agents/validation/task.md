...
Use empty arrays for contribution types that do not apply. IDs must be globally unique and should be prefixed with `validation.`.
...
```json
{
  ...
  "exports": [
    {
      "id": "validation.export.ExampleSymbol",
...
- manifest.agent is exactly "validation".
- Manifest IDs are unique and prefixed with "validation." where possible.
